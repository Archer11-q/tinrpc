...
InputBuffer::InputBuffer(size_t max_frames) : max_frames_(max_frames) {
}

auto InputBuffer::FindFrame(uint32_t frame_no) -> decltype(buffer_.begin()) {
    // deque 按 frame_no 升序，二分查找
    auto it = std::lower_bound(buffer_.begin(), buffer_.end(), frame_no,
...
std::unordered_map<std::string, std::vector<uint8_t>> InputBuffer::GetInput(uint32_t frame_no) {
