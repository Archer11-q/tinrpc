...
FrameSyncManager::FrameSyncManager(TimerManager* timer, InputBuffer* input_buffer,
                                   int tick_interval_ms, size_t history_size)
    : timer_(timer)
...
static void RescheduleTick(TimerManager* timer, int interval_ms, FrameSyncManager* self) {
...
void FrameSyncManager::OnPlayerInput(uint32_t frame_no, const std::string& player_id,
...
std::vector<FrameSyncManager::FrameRecord> FrameSyncManager::GetCatchUpFrames(
    uint32_t client_frame_no) const {
