...
MoveDir ParseMoveDir(const std::vector<uint8_t>& input) {
    ...
    uint8_t val = input[0];
    if (val >= 1 && val <= 4)
        return static_cast<MoveDir>(val);
    return MoveDir::NONE;
}
...
GameState tickLogic(const std::unordered_map<std::string, std::vector<uint8_t>>& inputs,
    ...
    std::sort(next.players.begin(), next.players.end(),
              [](const PlayerPos& a, const PlayerPos& b) { return a.player_id < b.player_id; });
...
// 辅助：在状态中查找玩家坐标
static bool FindPlayerPos(const GameState& state, const std::string& id, int32_t& x, int32_t& y) {
    for (auto& p : state.players) {
        if (p.player_id == id) {
            x = p.x;
            y = p.y;
...
void ReconcileState(GameState& client_state, const GameState& authoritative, float alpha) {
    for (auto& ap : authoritative.players) {
        // 在客户端状态中找该玩家
        auto it = std::find_if(client_state.players.begin(), client_state.players.end(),
                               [&](const PlayerPos& p) { return p.player_id == ap.player_id; });
