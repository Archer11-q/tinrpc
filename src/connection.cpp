...
Connection::Connection(int fd, EventLoop* loop, FrameCallback cb, DisconnectCallback on_disconnect)
    : loop_(loop), frame_callback_(std::move(cb)), on_disconnect_(std::move(on_disconnect)) {
    fd_ = fd;
}
...
void Connection::Send(const std::vector<uint8_t>& data) {
    ...
    if (write_buffer_.empty()) {
        ...
        if (n > 0) {
            // 部分发送：追加未发送部分
            write_buffer_.insert(write_buffer_.end(), data.begin() + n, data.end());
        } else {
...
void Connection::OnWrite() {
    while (!write_buffer_.empty()) {
        ...
        ssize_t n = send(fd_, write_buffer_.data() + write_offset_, remaining, MSG_NOSIGNAL);
        if (n > 0) {
