...
public:
    ...
    // 加入房间。失败时 .code 对应：
    //   ERR_ROOM_NOT_FOUND / ERR_ROOM_FULL / ERR_ROOM_NOT_JOINABLE
    //   / ERR_PLAYER_ALREADY_IN_ROOM（已在房间或已在其他房间）
    Result JoinRoom(const std::string& room_id, const std::string& player_id);

    // 离开房间。失败时 .code 对应：
    //   ERR_ROOM_NOT_FOUND / ERR_PLAYER_NOT_IN_ROOM
    // 离开后若房间为空，GameRoom 内部自动标记 DESTROYED
    Result LeaveRoom(const std::string& room_id, const std::string& player_id);

    // 开始游戏。失败时 .code 对应：
    //   ERR_ROOM_NOT_FOUND / ERR_NOT_OWNER / ERR_WRONG_ROOM_STATE
    Result StartGame(const std::string& room_id, const std::string& requester_id);

    // ---- 带通知的操作（成功后自动广播事件） ----

    // 加入房间 + 自动广播 PlayerJoinNtf（排除加入者自身）
    Result JoinRoomAndNotify(const std::string& room_id,
                             const std::string& player_id,
                             Broadcast* broadcast);

    // 离开房间 + 自动广播 PlayerLeaveNtf 给剩余玩家
    Result LeaveRoomAndNotify(const std::string& room_id,
                              const std::string& player_id,
                              Broadcast* broadcast);

    // 开始游戏 + 自动广播 GameStartNtf 给所有人
    Result StartGameAndNotify(const std::string& room_id,
