...
/**
 * @brief SnapshotManager — 快照管理器（帧同步断线重连基础）
 *
 * 职责：
 ...
 *
 * 使用方式：
 *   FrameSyncManager::Tick() 后 → tickLogic → SaveSnapshot(new_state)
...
public:
    /** @brief 构造函数
     *  @param max_snapshots 最大快照数（默认 60 帧 ≈ 3 秒 @20fps）
     */
    ...
    /** @brief 保存当前帧的游戏状态快照
     *  @param frame_no 帧号（应与 GameState.frame_no 一致）
     *  @param state    该帧的完整游戏状态
     */
    void SaveSnapshot(uint32_t frame_no, const GameState& state);

    /** @brief 获取指定帧的游戏状态快照
     *  @param frame_no 帧号
     *  @return 快照指针，nullptr 表示快照不存在（已被淘汰或从未保存）
     */
    const GameState* GetSnapshot(uint32_t frame_no) const;

    // ---- 快照回滚（断线重连入口） ----

    /** @brief 从指定帧的快照恢复游戏状态（断线重连入口）
     *  @param frame_no 客户端最后同步的帧号
     *  @return 该帧的完整状态副本（快照不存在时返回 frame_no=0 的空状态）
     *
     *  @note 当前仅返回快照的裸副本。完整流程需要：
