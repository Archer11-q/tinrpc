...
static bool JsonDecode(const std::string& json, TestData& out) {
    ...
    if (pos != std::string::npos) {
        ...
        if (end != std::string::npos) out.name = json.substr(pos, end - pos);
    }
    ...
    if (pos != std::string::npos) {
        ...
        if (end != std::string::npos) out.data = json.substr(pos, end - pos);
    }
...
static std::vector<uint8_t> TlvEncode(const TestData& d) {
    ...
    if (!d.name.empty()) ser.WriteString(d.name);
    if (!d.data.empty()) ser.WriteString(d.data);
...
static bool TlvDecode(const std::vector<uint8_t>& bytes, TestData& out) {
    ...
    if (!user_id || !balance || !age || !level || !score || !active) return false;
...
static BenchResult RunTlvBench(const PayloadConfig& cfg) {
    ...
    r.decode_correct = (output.user_id == input.user_id && output.balance == input.balance &&
                       ...
                       std::abs(output.score - input.score) < 0.01 &&
...
int main() {
    ...
    printf("%-30s %8s %12s %12s %12s %12s\n",
           "场景", "体积", "编码(ns)", "解码(ns)", "编码MB/s", "解码MB/s");
    printf("%s\n", std::string(90, '-').c_str());
    ...
    for (int i = 0; i < kNumPayloads; i++) {
        ...
        printf("  TLV:   体积=%4zu B | 编码=%8.0f ns | 解码=%8.0f ns | 编码=%7.1f MB/s | 解码=%7.1f MB/s  %s\n",
               tlv.serialized_bytes, tlv.encode_ns, tlv.decode_ns,
               tlv.encode_mbps, tlv.decode_mbps,
               tlv.decode_correct ? "✅" : "❌");

        auto json = RunJsonBench(cfg);
        printf("  JSON:  体积=%4zu B | 编码=%8.0f ns | 解码=%8.0f ns | 编码=%7.1f MB/s | 解码=%7.1f MB/s  %s\n",
               json.serialized_bytes, json.encode_ns, json.decode_ns,
               json.encode_mbps, json.decode_mbps,
               json.decode_correct ? "✅" : "❌");
        ...
        printf("  体积:  TLV 是 JSON 的 %.1f%%（节省 %.1f%%）\n",
               (1.0 / volume_ratio) * 100.0, (1.0 - 1.0 / volume_ratio) * 100.0);
