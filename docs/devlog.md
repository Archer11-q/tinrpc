...
## v0.10 — 匹配队列：为什么不用 `std::priority_queue`（2026-07-17）

### 背景

实现匹配系统时，需要维护一个按 ELO 分数排序的玩家池，并支持三个操作：
1. 入队（插入新玩家）
2. 离队（玩家取消匹配）
3. **查找最近 ELO 分数的对手**（匹配核心逻辑）

### 选项对比

| 数据结构 | 入队 | 离队 | 找最近邻居 | 适用场景 |
|---------|:--:|:--:|:--:|------|
| `std::priority_queue` | O(log n) | ❌ 不支持 | ❌ 只能取堆顶 | 取最大/最小值 |
| `std::set` (红黑树) | O(log n) | O(log n) | O(log n) `lower_bound` | 需要按键查找 |
| `std::vector` + 二分 | O(n) 插入 | O(n) 删除 | O(log n) `lower_bound` | 小数据量 |

### `priority_queue` 的具体问题

`priority_queue` 只暴露 `top()` / `push()` / `pop()`，三个致命缺陷：

1. **无法找最近邻居**：堆的堆顶是极值（最大或最小），但匹配需要找"分差最小的对手"——这是一个邻近搜索问题，堆不支持。要遍历所有元素才能找到，O(n) 且无法直接访问内部容器。

2. **无法随机离队**：玩家主动取消匹配时，需要从池中删除该玩家。`priority_queue` 没有 `erase` 操作，只能额外维护一个"已删除"集合做惰性清理，增加复杂度。

3. **`std::set` vs `std::vector`**：`set` 的 O(log n) 理论更优，但红黑树节点分散在堆内存中，遍历缓存不友好。匹配池规模通常在 10~100 人，`vector` 的 O(n) 插入在百级数据量下差距可忽略，且连续内存对 CPU 缓存友好。

### 最终决策：`std::vector` + `std::lower_bound`

- 入队：二分查找插入位置 → O(log n) 查找 + O(n) 移动（百级数据量可接受）
- 离队：线性扫描 player_id → O(n)，找到后 erase
- 匹配：`lower_bound` 定位自己的分数 → 向两侧扫描找分差最小且在放宽范围内的对手

**原因总结**：匹配系统不是"取最高分玩家"，而是"找和自己的分数最接近的玩家"。这决定了数据结构是邻近搜索而非极值查询，`priority_queue` 的语义从根本上不匹配。
